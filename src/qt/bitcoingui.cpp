...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a INTI address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and inticoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your INTI addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified INTI addresses"));
    ...
    openAction->setStatusTip(tr("Open a inticoin: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible INTI command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && qtumVersionChecker->newVersionAvailable())
        {
            ...
            QString message(tr("New version of INTICOIN wallet is available on the INTICOIN source code repository: <br /> %1 <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to INTICOIN network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
